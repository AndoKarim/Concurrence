...
Point Motor::changePosition(int x, int y) {
  int XAzimut = plateau.getXAzimut();
  int YAzimut = plateau.getYAzimut();
  int deltaX = abs(XAzimut - x);

  if (y == YAzimut)
    return Point(x - 1, y);

  if (y < YAzimut && deltaX > 1)
    return Point(x - 1, y + 1);

  if (y > YAzimut && deltaX > 1)
    return Point(x - 1, y - 1);

  if ((y < YAzimut && deltaX > 1) && (x - 2 == 127))
    return Point(x, y + 1);

  if ((y > YAzimut && deltaX > 1) && (x - 2 == 127))
    return Point(x, y - 1);

  if (y < YAzimut && deltaX == 0)
    return Point(x, y + 1);
